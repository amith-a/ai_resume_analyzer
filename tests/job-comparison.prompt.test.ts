...
describe("jobComparisonPrompt", () => {
  ...
  it("4. system prompt enforces fact grounding and prevents fabricated information", async () => {
    ...
    assert.ok(
      systemContent.includes("Fact Grounding"),
      "Must include Fact Grounding rule"
    );
    assert.ok(
      systemContent.includes("No Fabrication"),
      "Must include No Fabrication rule"
    );
    assert.ok(
  ...
  it("5. system prompt clearly instructs on all schema fields and empty list defaults", async () => {
    ...
    assert.ok(systemContent.includes("improvementSuggestions"), "Must reference improvementSuggestions");
  ...
  it("6. system prompt enforces qualitative overallFit and forbids numeric scores or percentages", async () => {
    ...
    assert.ok(
      systemContent.includes("numeric scores"),
      "Must explicitly forbid numeric scores"
    );
    assert.ok(
      systemContent.includes("percentages"),
      "Must explicitly forbid percentages"
    );
  });
  ...
  it("7. system prompt addresses prompt injection defenses and passive data treatment", async () => {
    ...
    assert.ok(
      systemContent.includes("Security & Safety"),
      "Must contain security rules"
    );
    assert.ok(
      systemContent.includes("passive data"),
      "Must state that text is passive data"
    );
  });
