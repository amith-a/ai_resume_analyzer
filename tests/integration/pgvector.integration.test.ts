...
// Helper to create a deterministic test vector with a spike at hotIndex
...
describe("PostgreSQL + pgvector Integration Tests", () => {
  ...
  it("1. verifies database connectivity and pgvector extension is installed and active", async (t) => {
    if (!isDbAvailable) {
      t.skip(skipReason);
      return;
    }
  ...
  it("2. verifies documents and document_chunks tables exist with expected schema", async (t) => {
    if (!isDbAvailable) {
      t.skip(skipReason);
      return;
    }
  ...
  it("3. verifies HNSW cosine vector index exists on document_chunks table", async (t) => {
    if (!isDbAvailable) {
      t.skip(skipReason);
      return;
    }
  ...
  it("4. inserts test document and document_chunks with 768-dimensional embeddings", async (t) => {
    if (!isDbAvailable) {
      t.skip(skipReason);
      return;
    }
  ...
  it("5. executes cosine distance (<=>) search returning nearest vectors first", async (t) => {
    if (!isDbAvailable) {
      t.skip(skipReason);
      return;
    }
  ...
  it("6. executes Euclidean L2 distance (<->) and negative inner product (<#>) queries", async (t) => {
    if (!isDbAvailable) {
      t.skip(skipReason);
      return;
    }
  ...
  it("7. combines relational/metadata filtering with vector similarity ordering", async (t) => {
    if (!isDbAvailable) {
      t.skip(skipReason);
      return;
    }
  ...
  it("8. verifies ON DELETE CASCADE removes chunks when parent document is deleted", async (t) => {
    if (!isDbAvailable) {
      t.skip(skipReason);
      return;
    }
