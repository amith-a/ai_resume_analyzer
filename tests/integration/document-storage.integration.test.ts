...
describe("Document & Chunks Storage Integration Tests", () => {
  ...
  it("1. inserts document and chunks, reads them back, and thoroughly verifies all fields and vector dimensions", async (t) => {
    if (!isDbAvailable) {
      t.skip(skipReason);
      return;
    }
  ...
  it("2. executes end-to-end storeDocumentWithChunks, reads back, and verifies cascade deletion", async (t) => {
    if (!isDbAvailable) {
      t.skip(skipReason);
      return;
    }
...
Master of Science in Software Engineering.
  ...
  it("3. executes vector retrieval (findChunksByDocumentIdOrderedBySimilarity) verifying document scoping, cosine distance ordering, and external document exclusion", async (t) => {
    if (!isDbAvailable) {
      t.skip(skipReason);
      return;
    }
  ...
  it("4. executes vector retrieval with similarity threshold (maxDistanceThreshold) verifying inclusion, exclusion, boundary, and empty results", async (t) => {
    if (!isDbAvailable) {
      t.skip(skipReason);
      return;
    }
  ...
  it("5. executes vector retrieval with metadata filtering (metadataFilter) verifying single/multiple conditions, exclusion, and combination with threshold", async (t) => {
    if (!isDbAvailable) {
      t.skip(skipReason);
      return;
    }
