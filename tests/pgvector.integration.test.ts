...
function createPerturbedVector(hotIndex: number, length: number = DEFAULT_VECTOR_DIMENSION): number[] {
...
describe("PostgreSQL + pgvector Integration Tests", () => {
  ...
  it("3. verifies HNSW cosine vector index exists on document_chunks table", async () => {
    ...
    assert.ok(indexRes.rows[0].indexdef.toLowerCase().includes("hnsw"), "Index definition must use hnsw");
    assert.ok(indexRes.rows[0].indexdef.toLowerCase().includes("vector_cosine_ops"), "Index definition must specify vector_cosine_ops");
  });
  ...
  it("4. inserts test document and document_chunks with 768-dimensional embeddings", async () => {
    ...
    const chunk1 = await pool.query(
      `INSERT INTO document_chunks (document_id, chunk_index, content, metadata, embedding)
       ...
       RETURNING id;`,
      [testDocumentId, 0, "Backend engineering with PostgreSQL and Node.js", JSON.stringify({ section: "experience" }), toVectorSql(vec0)]
    );
    ...
    const chunk2 = await pool.query(
      `INSERT INTO document_chunks (document_id, chunk_index, content, metadata, embedding)
       VALUES ($1, $2, $3, $4, $5::vector)
       RETURNING id;`,
      [testDocumentId, 1, "TypeScript architecture and database query optimization", JSON.stringify({ section: "skills" }), toVectorSql(vec1)]
    );
    ...
    const chunk3 = await pool.query(
      `INSERT INTO document_chunks (document_id, chunk_index, content, metadata, embedding)
       VALUES ($1, $2, $3, $4, $5::vector)
       RETURNING id;`,
      [testDocumentId, 2, "Visual branding and graphic illustration", JSON.stringify({ section: "hobbies" }), toVectorSql(vec2)]
    );
  ...
  it("5. executes cosine distance (<=>) search returning nearest vectors first", async () => {
    ...
    assert.ok(Math.abs(result.rows[2].distance - 1.0) < 1e-5, "Orthogonal vector cosine distance should be ~1.0");
  });
  ...
  it("6. executes Euclidean L2 distance (<->) and negative inner product (<#>) queries", async () => {
    ...
    assert.ok(Math.abs(ipResult.rows[0].neg_ip - (-1.0)) < 1e-5, "Normalized vector dot product should be -1.0 for <#>");
  });
  ...
  it("8. verifies ON DELETE CASCADE removes chunks when parent document is deleted", async () => {
    ...
    await pool.query(
      ...
      [testDocumentId2, 0, "Temporary chunk to verify cascade delete", toVectorSql(createTestVector(10))]
    );
